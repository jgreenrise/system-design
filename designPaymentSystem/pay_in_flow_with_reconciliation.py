...
with Diagram("Pay-in flow with Reconciliation", direction="LR", graph_attr=graph_attr):
    customer = Person(
    ...
    with SystemBoundary("External Systems"):
        ...
        reconciliation_service = Container(
            name="Reconciliation Service",
            technology = "J2EE Service",
            description="Orchestrate reconciliation Activities"
        )
    ...
    payment_executor >> Relationship("5. Calls an external PSP to process the credit card payment.") >> psp
    psp >> card_schemes
    ...
    psp >> Relationship("1. Send Settlement file.", color="Green", style="bold") >> reconciliation_service
    ledger_Service >> Relationship("Send payments file.", color="Green", style="bold") >> reconciliation_service
